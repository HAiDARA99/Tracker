...
class CreationTrackerVC: UIViewController {
    ...
    private let trackerRecord = TrackerRecordStore()
    private let trackerCategory = TrackerCategoryStore()
    ...
    @objc func createButtonTapped() {
        ...
        let newTracker = Tracker(id: UUID(), name: name, color: color, emoji: emoji, schedule: selectedWeekDays)
        do {
            try trackerStore.addTracker(newTracker, categoryTitle: "По умолчанию")
            trackerCreationDelegate?.passingTracker(newTracker, "По умолчанию")
            dismiss(animated: true)
        } catch {
...
extension CreationTrackerVC: TableViewCollectionViewCellDelegate {
    ...
    func didSelectCategoryRow() {
        navigationController?.popToViewController(ScheduleVC(), animated: true)
    }
}

extension CreationTrackerVC: ColorCellDelegate {
    func didSelectColor(_ color: UIColor) {
        selectedColor = color
    }
}

extension CreationTrackerVC: EmojiCellDelegate {
    func didSelectEmoji(_ emoji: String) {
        selectedEmoji = emoji
    }
}

extension CreationTrackerVC: TextFieldCollectionViewCellDelegate {

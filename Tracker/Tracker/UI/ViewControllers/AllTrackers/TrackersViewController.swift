...
final class TrackersViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupCollectionView()
        filterTrackers(for: selectedDate, category: TrackerCategory(title: "По умолчанию", trackers: []))
        updateUI()
    }
    ...
    private func filterTrackers(for date: Date, category: TrackerCategory?) {
        let trackers = try? trackerStore.fetchTrackers(forDate: date)
        print("Fetched trackers for \(date):", trackers ?? [])
        
        if let trackers, !trackers.isEmpty {
            let categoryTitle = category?.title ?? "По умолчанию"
            visibleCategories = [TrackerCategory(title: categoryTitle, trackers: trackers)]
        } else {
            visibleCategories = []
        }
...
extension TrackersViewController: TrackerStoreDelegate {
    func trackerStoreDidUpdate() {
        filterTrackers(for: selectedDate, category: nil)
    }
}
